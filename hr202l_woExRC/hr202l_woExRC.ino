...
Schematics: Connect each of HR202L pins to the ESP32 GPIO 32 and 33.
                               ...
                               https://bokunimo.net/
*******************************************************************************/
float _hr202_temp_offset = -20.0;               // 温度値の補正用
uint8_t _hr202_PIN_A1    = 32;                  // HR202Lを接続
...
void setup(){                                   // 起動時に一度だけ実行する関数
    ...
    hr202_Setup(_hr202_PIN_A1,_hr202_PIN_A2);   // 湿度センサの初期化
}

void loop(){
    float temp = hr202_getTemp();               // 温度を取得して変数tempに代入
    float hum  = hr202_getHum();                // 湿度を取得して変数humに代入
    Serial.print("Temp = ");
    Serial.print(temp,1);
    Serial.print(", Humi = ");
